...
function startAttract(canvas, world) {
  ...
  const io = new IntersectionObserver((entries) => {
    ...
    if (visible) {
      ...
      // a video-backed attract (onairAttract) keeps decoding/playing off-screen
      // otherwise — canvas-only attract functions have zero cost once raf stops,
      // this keeps a <video>-backed one matching that
...
function candyTrisAttract(ctx, w, h, t, world, s) {
  ...
  videoAttract(ctx, w, h, t, s, 'assets/footage/candy-tris-attract.mp4?v=3', '#110722');
}
...
function theCountAttract(ctx, w, h, t, world, s) {
  // Real recorded gameplay, not a drawn mini-scene (Osimo 2026-09-05: "renew the cassette
  // with actual game play now that we have some"). Recorded by
...
function onairAttract(ctx, w, h, t, world, s) {
  ...
  // the current frame of a real playthrough clip each tick, cover-fit to the cassette box
  // (crops overflow rather than letterboxing, matches CSS background-size:cover). Recorded
  // via tools/show/record_onair_gameplay.py — genuine hotspot clicks, genuine walking,
  ...
  const v = s.video;
  ctx.fillStyle = '#160f06'; ctx.fillRect(0, 0, w, h);
...
function angryWormsVideoAttract(ctx, w, h, t, world, s) {
  // v5 (2026-09-02): v4 caught the slingshot mid-bug — the near fork arm only drew while
  // gameState was 'playing', so attract (which is all the cassette records) filmed half a Y
  // (Osimo: "in the cassette the catapult half rendered"). Fixed in the game, re-recorded.
  ...
  videoAttract(ctx, w, h, t, s, 'assets/footage/angry-worms-attract.mp4?v=5', '#100604');
}
